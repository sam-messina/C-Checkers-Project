...
vector<int*> PieceH::get_moves(int** map){
	...
	int move_counter=0;			// keeps track of the number of jumps

	int* take_list;				// initialse an array to store the ID's of taken pieces
	take_list=new int[7];
	for(int i=0;i<7;i++){
		take_list[i]=0;
	}
	...
	for(int j=0;j<unfinished.size();j++){ // go through the moves
		...
		target_square[1] = unfinished[j][1]-1; // y
		...
		for(int i=0;i<2;i++){ // repeat for left and right
				...
				if(j==0&&on_board(target_square[0],target_square[1])==true && board[target_square[0]][target_square[1]]==0){// check for empty case
				...
				// 					move is on the board &&	there is an enemy on this square && a gap behind
					...
					movelist[move_counter][1]=target_square[1]-1; // UP
